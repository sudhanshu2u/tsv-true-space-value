...
const floorPlanTool: Anthropic.Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      estimatedCarpetAreaSqFt: { type: "number", description: "RERA carpet area from printed table if available, else visual estimate" },
      passageAreaPercent: { type: "number", description: "Passage/corridor as % of carpet area, calculated from room schedule table if available" },
      ...
      notes: { type: "string", description: "Include exact RERA areas, room dimensions from schedule, facing from compass, unit identifier" },
      facingFromPlan: { type: "string", enum: ["North", "South", "East", "West", "NE", "NW", "SE", "SW"], description: "Facing direction from key plan compass — omit if not legible" },
      projectIdentifier: { type: "string", description: "Unit identifier from plan header e.g. Wing A Flat 01 — omit if not visible" },
    },
    required: [
      "bedrooms",
      "bathrooms",
